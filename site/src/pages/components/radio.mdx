...
`critical`. `tone` tints the **whole option** — the fill, the label, the hint, and
(for a tinted tone) the unselected ring border (a light shade of the fill that
darkens on hover and active, like the neutral ring; `neutral` keeps a grey ring).
Set it on the `RadioGroup` to colour every option, or on a single option to
override just that one. Any literal CSS colour works too, for a one-off custom tone.
...
<Preview>
  <RadioGroup
    ...
    options={[
      ...
      { value: 'secondary', label: 'Secondary', priority: 'secondary' },
    ]}
  />
</Preview>
...
`priority` sets how the selected ring is painted. `primary` (the default) fills
the ring with the tone colour and centres a white dot. `secondary` leaves the ring
unfilled and draws the border + dot in the tone colour — an outlined look. Set it
on the `RadioGroup` for the whole set, or on a single option to override one.
...
| `onAnyChange(e, attrs)` | **after** selection applies | — | `{ value, name }` |

Reach for **`onStateChange`** to intercept/veto or to filter on `reason` (reset vs
restore vs user); for the everyday "react to the new value", use **`onAnyChange`**.
...
```tsx folded
<RadioGroup onAnyChange={(_e, { value }) => save(value)} options={…} />
```
...
**`size`** the dimensions + type, **`priority`** the fill-vs-outline look. The focus
ring is the global [`--focus-ring`](/colors/#focus-ring).
...
is `::after`**, the label/hint are the `a-radio-label` / `a-radio-hint` children, and
the options sit in a plain `<a-radio-list>`. Target them with ordinary CSS (an
un-layered rule beats `@layer anta` without `!important`):

```css folded
/* square the ring, resize the dot, lay options out in a grid */
a-radio::before { border-radius: 4px; }
a-radio::after { inline-size: 8px; block-size: 8px; }
a-radio-group a-radio-list { display: grid; grid-template-columns: 1fr 1fr; }
```
