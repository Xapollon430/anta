...
`tone` also accepts any literal CSS color for a one-off tag. The fill and
border become alpha tints of that color and the text deepens it to a
readable foreground — the same recipe as the named secondary tones.
...
tag. Each tone sets `--tag-text` (the theme-aware `--text-3-{tone}` token)
and `--tag-tint` (the hue the fill + border are tinted from), so setting
`--tag-tint` alone re-tints the whole pill.

| Token | Description |
|---|---|
| `--tag-text` | Primary text (`value`) and icon color. |
| `--tag-tint` | Hue the fill + border are tinted from. Set it to recolor both at once. |
| `--tag-bg` | Background fill. Defaults to `--tag-tint` at 7–20% alpha (tone- and theme-dependent). |
| `--tag-border` | Outer hairline. Defaults to `--tag-tint` at 15–25% alpha. |
