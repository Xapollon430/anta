...
| `--input-label` | Label color. Defaults to `--text-3`. |
| `--input-placeholder` | Placeholder color. Defaults to `--text-4`. |
| `--input-adornment` | Color of `leading` / `trailing` content — icons and plain text (a slotted `<Button>` keeps its own). Defaults to `--text-4`. |
