...
keyboard navigation, and listbox ARIA. The point: the field already provides the
slots and states such a pattern needs.
...
Read `validationMessage` and pass it to **`error`** — the field goes red, shows
the warning glyph, and the message replaces the hint. Mark the form `noValidate`
so the browser's default bubbles step aside for Anta's styling. Try submitting
empty, or with a bad email / out-of-range age.
