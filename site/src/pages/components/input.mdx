...
`<a-input>` is shadow-DOM, so for everything else style its **parts** —
`::part(field | input | label | leading | trailing | hint)` — with plain CSS;
`:focus-within::part(field)` reaches the focused state. `light-dark()` works too
(Anta sets `color-scheme` from its theme toggle). The `.fancy` class is just for the
demo — a borderless pill that fills dark-grey and darkens on focus in light mode,
