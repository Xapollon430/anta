...
`warning-triangle`, `success` → `circle-check`, `info` → `info`). Override it
with **`statusIcon`** (any shape), or drop it with `statusIcon={false}`.
...
width (`0.5px` → `1px` when a `status` is set) and placement never change the box size —
...
| `--input-border`<br />`--input-border-hover` | Border color at rest / hover. Default `--border-2` / `--border-1`. A `status` re-points both to its tone's `--border-1-*` role token. |
...
| `--input-hint` | Hint message + glyph color. Defaults to `--text-3`; a `status` re-points it to its tone's `--text-2-*` role token. |
