...
// Pinned under the anchor by default.
<Button label="Save">
  <Tooltip>Saves your work</Tooltip>
</Button>

// Prefer the top side (auto-flips down when there's no room).
...
| `--tooltip-padding` | Inner padding of the bubble. Defaults to `5px 8px`. |

For anything the tokens don't cover, the bubble surface is exposed as a shadow
**part**, so you can reach it directly with `::part(bubble)`:
...
(`::part` styles the bubble box itself; to restyle the *text content* you pass
in, put a class on it and style that, as above.)
