...
`<a-button>` is light-DOM, so one-offs are plain CSS — spacing, the loading stripe,
transitions (an un-layered rule beats `@layer anta` without `!important`):

```css folded
a-button { padding-inline: 12px; transition-duration: 0; }
```

Don't reach for the resolved `--button-fg` / `--button-bg`: they're recomputed per
state, so setting one only catches a single state — use `tone` for colour.
