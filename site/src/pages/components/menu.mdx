...
**Menu items aren't tokenized.** `a-menu-item` is light DOM, so style it — and its
`a-icon` / `kbd` / `a-menu-item-label` children — directly (e.g.
`a-menu-item { font-size: 13px }` or a class). The `tone` attribute covers the
semantic palette.
