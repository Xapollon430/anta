...
(`detail: { open, previousState }`, the new state as a boolean and the old as a
string) to observe it, or pass **`state="opened" | "closed"`** to control it
(submenus stay uncontrolled). You can also grab a `ref` and call `.open()` /
`.close()` / `.toggle()`.
...
For programmatic open/close from app state, drive the `open` attribute (or call
`.close()` on the element via a `ref`).
