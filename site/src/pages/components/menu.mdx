...
Open state is uncontrolled by default — pass **`onOpenChange`** (or listen for
the raw **`openchange`** event, `detail: { open, previousState }`) to observe it,
or pass **`state="opened" | "closed"`** to control it, flipping `state` from your
`onOpenChange` handler (submenus stay uncontrolled). You can also grab a `ref`
...
For programmatic open/close from app state, drive the `state` attribute
(`"opened"` / `"closed"`) and react via `onOpenChange`, or grab a `ref` and call
`.open()` / `.close()` / `.toggle()`.
