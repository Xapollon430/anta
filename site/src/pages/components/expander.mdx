...
Or just **highlight the title** — recolour the header on hover with
`::part(summary):hover`; the slotted title inherits the colour, so the whole header
eases to it:

<Preview direction="column" gap="8px">
  <Expander class="title-highlight" title="Hover the title" priority="tertiary">
    <Text>The title eases to the brand color on hover via ::part(summary).</Text>
  </Expander>
  <style is:inline>{`
    a-expander.title-highlight::part(summary):hover { color: var(--text-1-brand); }
  `}</style>
</Preview>
...
```css folded
a-expander::part(summary):hover { background: var(--bg-3); color: var(--text-1); }
a-expander::part(summary)::before { transform: scale(1.2); } /* the chevron */
a-expander::part(content) { padding-block: 12px; }
```
