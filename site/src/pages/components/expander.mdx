...
**Highlight the title on hover — the one unusual case.** A plain
`::part(summary):hover { color }` *won't* recolour the title, and it's worth knowing
why. The title you pass is **slotted light DOM** (`<a-expander-summary slot="title">`),
projected into a `<slot>` *inside* the `::part(summary)` button — and `::part()` only
matches the shadow element and its shadow descendants, **never slotted nodes**, so there
is no part to target for the title. (That `color` *does* reach the chevron, which is
`currentColor`-based — so it recolours the arrow, not the text.) The title also pins its
own `color`, so it ignores anything inherited from the part anyway. The element instead
drives the title colour through one **inherited custom property**, flipped on hover —
routed that way to dodge a Chromium bug where `::slotted` styles don't invalidate on an
ancestor's `:hover`. So the hook is the **`--expander-text-hover`** variable, not a part:
...
<style>{`
  a-expander.code-like {
    ...
    --expander-text-hover: var(--text-1);  /* title brightens on hover (see the note above) */
