...
The generator emits one `Sticker{Name}` per sub-folder plus an `index.ts` barrel, each importing `StickerLottie` from `@antadesign/anta/sticker-runtime` — a public subpath of the package, so it resolves wherever the consumer's bundler finds anta. Tree-shaking, ARIA, and the `paused` semantics behave the same as anta's built-in stickers; the only difference is the import root.
