...
export class AButtonElement extends HTMLElementBase {
  constructor() {
    ...
    // Structural defaults only — all visual styling lives in
    // src/elements/a-button.css and targets the host (`a-button`)
    // directly, so consumers can reach it from outside the shadow.
    style.textContent = `
      :host {
        display: inline-flex;
        gap: var(--button-gap, 0.5ch);
        align-items: center;
        overflow: hidden;
        user-select: none;
        cursor: pointer;
...
function handleKeyDown(e: KeyboardEvent) {
  ...
  const el = (e.target as HTMLElement).closest('a-button') as AButtonElement | null
  if (!el) return
  e.preventDefault()
  el.click()
}
...
function handleClick(e: MouseEvent) {
  ...
  if (customEvent) {
    el.dispatchEvent(new CustomEvent(customEvent, { bubbles: true, cancelable: true }))
  }
  ...
  if (type === 'submit') {
    ...
    form.dispatchEvent(new CustomEvent('submitdetailed', {
      bubbles: true,
      cancelable: true,
      detail: {
