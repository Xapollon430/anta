...
When naming components, props, CSS variables, internal class names, or suggesting patterns, reference established design systems: Material Design, shadcn/ui, Radix UI, Carbon Design System, Shopify Polaris Web Components, Ant Design, Atlassian Design System, Base UI (base-ui.com), and Blueprint. Prefer terminology and conventions already used by these libraries over inventing new ones.
...
- **No comments in shipped CSS.** `a-{name}.css` / `*.module.css` files are copied verbatim to `dist`, and shadow `<style>` strings are injected into every consumer document at runtime — comments in either ship to consumers. Keep them comment-free; put the explanation, condensed, in the companion `.ts`/`.tsx` instead (a "Styling notes" section in the class/wrapper docstring for the external CSS, or a TS comment directly above the shadow style string). `tokens.css` / `reset.css` are exempt — consumers read those files directly as theming reference.
