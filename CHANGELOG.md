...
- **Selected buttons gain an inset ring.** `selected` now adds `box-shadow: inset 0 0 0 1px currentColor` — a 1px outline in the current foreground color — on top of the existing pressed/active look. Declared after the priority blocks so it survives their `box-shadow` cancels regardless of priority.
- **Secondary buttons gain a hairline edge.** The default `secondary` priority now carries `box-shadow: 0 0 1px color-mix(in oklch, currentColor, transparent 40%)` — a 1px outline in the current foreground tone at 60% alpha — giving the tinted chip a subtle border. `primary`, `tertiary`, and `quaternary` explicitly cancel it (`box-shadow: none`).
- **Quaternary button labels are slightly heavier.** `priority="quaternary"` sets `font-weight: 425` (vs the `450` shared default) so the faded quaternary text holds up at its lower opacity.
