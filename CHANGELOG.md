...
- **New `Radio` / `RadioGroup` components** (`<a-radio>` / `<a-radio-group>`) — a single-select radio control, anta's first form-associated element. `RadioGroup` coordinates a set of `Radio` options: exactly one selected at a time, with arrow-key navigation and roving tabindex. Registered via the `@antadesign/anta/elements` barrel.
  - **State** — follows the shared `state` contract (like `Checkbox` / `Expander`): controlled (`value` + `onStateChange`) or uncontrolled (`defaultValue`). The element carries `state` / `default-state` and fires a **cancelable `statechange` *before* applying** — so an uncontrolled consumer can veto a pick with `event.preventDefault()`, and a controlled one answers by updating `value`. All selection state lives in `<a-radio-group>`, so the wrappers stay stateless and the elements work in plain HTML / any framework.
  - **Forms** — `<a-radio-group>` is a form-associated custom element: give it a `name` and the selected option's `value` submits with a wrapping `<form>` like a native radio group. Form **reset** restores the `defaultValue` selection, and the selection survives bfcache / autofill restore.
  - **Variants** — `tone` (six named: `brand` default, `neutral`, `info`, `success`, `warning`, `critical`, plus **any literal CSS colour**) and `size` (`small` / `medium` default / `large`) set on the group cascade to every option (a `Radio` can override its own); `orientation` (`vertical` default / `horizontal`) sets the layout + arrow-key axis; `disabled` on a `Radio` or the whole `RadioGroup`. Named tones track light/dark mode automatically; a custom colour keeps its hue + chroma and pins lightness to the brand fill curve (same derivation as `Checkbox`).
  - **Accessibility** — the `RadioGroup` wrapper sets `role="radiogroup"` on the group and `role="radio"` on every option; each `<a-radio>` publishes its own `aria-checked` through `ElementInternals` (off the DOM), driven by the `selected` property the group sets — so it stays live through arrow-key navigation in uncontrolled mode. A `label` is wired as the group's accessible name via `aria-labelledby`. `disabled` on either layer reflects to `aria-disabled`. Follows the [WAI-ARIA radio-group pattern](https://www.w3.org/WAI/ARIA/apg/patterns/radio/).
  - **Structure** — `<a-radio-group>` uses a minimal shadow root with three slots: a `<a-radio-label slot="label">`, the `<a-radio>` options (default slot), and a `<a-radio-hint slot="hint">`. The default slot's `slotchange` is how the group tracks its option set (so dynamically added/removed radios stay in sync). The wrapper composes the label/hint from `label` / `hint`; hand-authors write the slotted children directly.
