...
- **Dark tinted button fills bumped (secondary, tertiary, custom tones).** The five named tinted tones (`brand` / `critical` / `info` / `success` / `warning`) step their dark-mode **secondary** fill to 20 → 25 → 30% alpha (rest / hover / active), from ~15 → 20 → 25%; their **tertiary** hover/active fills move in lockstep to 20 → 25%. Custom (color-literal) tones get the same dark ramp via the `--_tone-bg-a-*` knobs (`0.20 / 0.25 / 0.30`). Neutral is unchanged (~9 → 14 → 18% secondary, ~8 → 13% tertiary).
